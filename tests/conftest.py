...
class ToolCapture:
    ...
    def tool(
        ...
        calls populate ``self.tools`` with the raw async functions, making them
        directly callable in tests without a live MCP server. Keyword arguments
        (e.g. ``readOnlyHint``, ``destructiveHint``) are accepted and ignored.
        """
...
def mock_client() -> MagicMock:
    ...
    ``get``, ``post``, ``delete``, and ``ws_command`` are ``AsyncMock``
    instances. Configure return values in each test via
    ``mock_client.get.return_value`` etc.
    """
...
def mock_ctx(mock_client: MagicMock) -> MagicMock:
    ...
    ``ctx.request_context.lifespan_context.client`` resolves to the
    ``mock_client`` fixture, matching the structure that tool functions
    expect when they retrieve the client via:
