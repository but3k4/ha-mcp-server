...
def tools() -> dict[str, Any]:
    """Register helper tools and return the tool dict."""

    capture = ToolCapture()
    helpers.register(capture)  # type: ignore[arg-type]
...
async def test_set_input_datetime_time_only(
    ...
    await tools["set_input_datetime"](
        ctx=mock_ctx, entity_id="input_datetime.alarm", time="07:30:00"
    )
    mock_client.post.assert_called_once_with(
...
async def test_set_input_datetime_full(
    ...
    await tools["set_input_datetime"](
        ...
        date="2025-01-01",
        datetime_str="2025-01-01 08:00:00",
    )
    mock_client.post.assert_called_once_with(
        "/api/services/input_datetime/set_datetime",
        {
...
async def test_start_timer_with_duration(
    ...
    await tools["start_timer"](
        ctx=mock_ctx, entity_id="timer.cooking", duration="00:05:00"
    )
    mock_client.post.assert_called_once_with(
        "/api/services/timer/start",
